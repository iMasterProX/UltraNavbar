...
### 분할화면 버그 수정
- 앱 즐겨찾기에서 앱 추가 후 분할화면 시도 시, UltraNavbar 자체가 분할화면에 열리는 버그 수정
...
### 앱 즐겨찾기 패널 개선
- 홈 화면에서 앱 즐겨찾기 열 때 흰색 배경 깜빡임 수정
- 앱 즐겨찾기 열 때 태스크바가 잠깐 보이는 현상 수정
- 그리드 레이아웃: weight 기반 균등 열 배분으로 아이콘 정렬 수정

### 기타
- '네비바앱스' → '앱 즐겨찾기'로 명칭 변경
- 키보드 단축키 액션 유형에서 '설정 열기' 옵션 숨김 (바로가기로 대체 가능)

---

## English

### App Favorites - New Feature
...
- Fixed UltraNavbar itself opening in split-screen after adding apps via App Favorites
...
### App Favorites Panel Improvements
- Fixed white background flash when opening App Favorites on home screen
- Fixed taskbar briefly appearing when App Favorites is opened
- Fixed grid layout alignment with weight-based equal column distribution

### Other
- Renamed 'NavbarApps' to 'App Favorites' for clarity
- Hidden 'Open Settings' option from keyboard shortcut action types (replaceable via Shortcuts)
